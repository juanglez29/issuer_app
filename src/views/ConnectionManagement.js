...
function ConnectionManagement() {
    ...
    useEffect(async () => {
        ...
        if (filter === "pendinng") {
            await axios.get('http://localhost:8021/myapi/connections/pending')
                .then(res => setList(res.data.connections_pending))
        }

        if (filter === "active") {
            await axios.get('http://localhost:8021/myapi/connections/active')
    ...
    async function receiveandaccept(event) {
        try {
            ...
            await axios.post('http://localhost:8021/myapi/connections/accept-invitation', { invitation_url: url })
